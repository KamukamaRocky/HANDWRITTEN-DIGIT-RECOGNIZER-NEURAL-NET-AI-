...
class Network():
    def __init__(self,sizes):
        ...
        self.weights=[ np.random.randn() for (x,y) in zip(self.sizes[:-1],self.sizes[1:])]
    def feedforward(self,a):
        for w,b in zip(self.weights,self.biases)
            a=self.sigmoid(np.dot(w,a)+b)
        return a
    def SGD(self,training_data,mini_batch_size,epochs,test_data=None):
    ...
    def update(self,mini_batch,eta):
        ...
        """ already nabla_b is a sum up of the changes..this the classic stochaistic gradient descent rule"""
